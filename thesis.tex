...
\begin{align*}
	...
	\envshell_{\tau, \Delta}(x,\vec x \mapsto v, \vec v) &= x,\vec x \mapsto \shell_{\tau}(v), \envshell_{\Delta}(\vec v)
\end{align*}

\begin{align*}
	\envholes_{\alpha \in \emptyset}() &= \emptyset \\
	\envholes_{\alpha \in \tau,\Delta}(x, \vec x \mapsto v, \vec v) &= \holes_{\alpha \in \tau}(v),\envholes_{\alpha \in \Delta}(\vec v)
\end{align*}
...
\begin{definition}
	...
	\begin{itemize}
		\item \(\delta_1\) and \(\delta_2\) both have type environment \(\Delta\), and
...
\begin{proof}
	...
	The shell condition is direct equality, so \(\eqpatD\) restricted soley to shells is clearly an equivalence relation.
	It remains to show that the \(\eqpatD\) conditions for holes form an equivalence relation for value environments.
...
\begin{lemma}
	...
	Let \(\delta_1, \delta_2\) be value environments both with corresponding type environment \(\Delta\), where \(\delta_1 \eqpatD \delta_2\).
...
\begin{theorem}
	Let \(g\) be a goal, \(\eta\) be a relation environment, and \(\delta_1, \delta_2\) be value environments with corresponding type environment \(\Delta\).
	...
	\(\den{g}(\eta, \delta_1) = \den{g}(\eta, \delta_2)\).
\end{theorem}
\begin{proof}
	Proof by induction on the goal structure of \(g\).
	\begin{itemize}
		\item \(g = \texttt{(conj \(g_1\) \(g_2\))}\).
			By the inductive hypothesis, \(\den{g_1}(\eta; \delta_1) = \den{g_1}(\eta; \delta_2)\) and \(\den{g_2}(\eta, \delta_2) = \den{g_2}(\eta; \delta_2)\)
			Thus \(\den{g_1}(\eta; \delta_1) * \den{g_2}(\eta; \delta_1) = \den{g_1}(\eta; \delta_2) * \den{g_2}(\eta; \delta_2)\), and so \(\den{g}(\eta, \delta_1) = \den{g}(\eta, \delta_2)\).
		...
		\item \(g = \texttt{(fresh ((\(x : \tau\))) \(g'\))}\).
			...
			By lemma \ref{lem:eqpat-extend}, for any \(\Delta \vdash v_1 : \sigma_1(\tau)\),
			there exists \(\Delta \vdash v_2 : \sigma_2(\tau)\) where \(\delta_1, x \mapsto v_1 \eqpat_{\Delta, \tau} \delta_2, x \mapsto v_2\).
			Similarly, \(|\sigma_2(\alpha)| \ge \#_\alpha g'(\Delta,\tau) \; \forall \alpha \in \Delta\), so for any \(\Delta \vdash v_2 : \sigma_2(\tau)\),
			there exists \(\Delta \vdash v_1 : \sigma_1(\tau)\) where \(\delta_1, x \mapsto v_1 \eqpat_{\Delta, \tau} \delta_2, x \mapsto v_2\).
			Thus \(\den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_1) = w_1 + \dots + w_n\),
			and \(\den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_2) = w'_1 + \dots + w'_m\),
			...
			We therefore have: \(\den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_1) = (r_1 + \dots) + \dots + (r_k + \dots)\),
			and \(\den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_2) = (r_1 + \dots\dots) + \dots + (r_k + \dots\dots)\).
			We require \(+\) to be idempotent.
			Thus: \(\den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_1 = r_1 + \dots + r_k + \den{\texttt{(fresh ((\(x : \tau\))) \(g'\))}}(\eta, \delta_2)\)
		\item \(g = \texttt{(== \(v_1\) \(v_2\))}\).
			...
			In the first case, \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_1) = 1\) and \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_2) = 1\).
			In the second case, \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_1) = 0\) and \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_2) = 0\).
			In either case, \(\den{g}(\eta, \delta_1) = \den{g}(\eta, \delta_2)\).
		\item \(g = \texttt{(=/= \(v_1\) \(v_2\))}\).
			By lemma \ref{lem:eqpat-substitution}, we either have \(\den{v_1}(\delta_1) = \den{v_2}(\delta_1)\) and \(\den{v_1}(\delta_2) = \den{v_2}(\delta_2)\),
			or \(\den{v_1}(\delta_1) \ne \den{v_2}(\delta_1)\) and \(\den{v_1}(\delta_2) \ne \den{v_2}(\delta_2)\).
			In the first case, \(\den{\texttt{(=/= \(v_1\) \(v_2\))}}(\eta, \delta_1) = 0\) and \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_2) = 0\).
			In the second case, \(\den{\texttt{(=/= \(v_1\) \(v_2\))}}(\eta, \delta_1) = 1\) and \(\den{\texttt{(== \(v_1\) \(v_2\))}}(\eta, \delta_2) = 1\).
			In either case, \(\den{g}(\eta, \delta_1) = \den{g}(\eta, \delta_2)\).
		\item \(g = \texttt{(\(R\) \(v\) \(\dots\))}\).
			% TODO: clarify this case
			We assume relation calls work as expected.
		\item \(g = \texttt{(factor \(k\))}\).
			\(\den{\texttt{(factor \(k\))}}(\eta, \delta_1) = k = \den{\texttt{(factor \(k\))}}(\eta, \delta_2)\)
	\end{itemize}
\end{proof}
