...
\begin{proof}
	...
	so we can choose holes \(h_1 \in \holes_{\alpha \in \tau}(\den{v_1}(\delta_1))\) at index \(i\), and \(k_1 \in \holes_{\alpha \in \tau}(\den{v_2}(\delta_1))\) both at index \(j\) where \(i = j\).
	...
	Because we know \(\envshell_\Delta(\delta_1) = \envshell_{\Delta}(\delta_2)\), we know holes \(h_2, k_2\) are at indices \(i = j\) within the value shells of \(v_1, v_2\).
	...
	Thus we must have \(\den{v_1}(\delta_2) \ne \den{v_2}(\delta_2)\).
\end{proof}
