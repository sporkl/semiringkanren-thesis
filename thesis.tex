...
\begin{theorem}
	...
	and \(\gamma : \Gamma\) satisfy for all valid \(\texttt{(\(R\) \(\vec v\))}\): \(\gamma(R)(\den{\vec v}(\delta_1)) = \gamma(R)(\den{\vec v}(\delta_2))\).
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item \(g = \texttt{(\(R\) \(\vec v\))}\).
			\(\den{\texttt{(\(R\) \(\vec v\))}}(\gamma;\delta_1;\sigma_1) = \gamma(R)(\den{\vec v}(\delta_1))\)
			\(\den{\texttt{(\(R\) \(\vec v\))}}(\gamma;\delta_2;\sigma_2) = \gamma(R)(\den{\vec v}(\delta_2))\)
			By hypothesis, these are equivalent.
		\item \(g = \texttt{(factor \(k\))}\).
			\(\den{\texttt{(factor \(k\))}}(\gamma; \delta_1; \sigma_1) = k = \den{\texttt{(factor \(k\))}}(\gamma; \delta_2; \sigma_2)\)
	\end{itemize}
\end{proof}

The condition on \(\gamma\) is quite restrictive.
...
We now aim to compile relation calls with large-enough arguments to call the smallest compatible relation.
While it remains necessary to keep monomorphized versions of relations with small-sized arguments,
this allows us to remove monomorphized versions of the relations with large arguments.
...
% TODO: define and prove correctness of compile function
