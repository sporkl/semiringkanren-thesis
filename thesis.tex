...
\begin{proof}
	...
	\begin{itemize}
		...
		\item \(g = \texttt{(\(R\) \(\vec v\))}\).
			...
			By the hypothesis that all relations in \(\gamma\) are large-enough, these are equivalent.
...
We now aim to compile monomorphized relation calls to instead call the just smallest large-enough relation.
...
In practice, implementation of this compilation scheme is verbose, so we reason here with a simplified/idealized version of the code.
...
\begin{align*}
	...
	& \quad \quad \quad \text{for \(i,j \in 1,\dots,\#_\alpha\Delta\)} \\
...
\begin{proof}
	...
	\(\delta_1 \eqpatD \delta_2\), so \(\envshell_\Delta(\delta_1) = \envshell_\Delta(\delta_2)\),
	so \(\texttt{(== \(\envshell_{\Delta}(\Delta_1)\) \(\envshell_{\Delta}(\Delta_2)\))}\) succeeds under \(\delta_1, \delta_2\).
	...
	Exactly one branch of the \(\texttt{disj}\) succeeds for each \(\alpha, i, j\) (and we can ignore holes which do not "exist" here), so all such \(\texttt{disj}\)s have weight 1.
	...
	Thus \(\texttt{(== \(\envshell_\Delta(\Delta_1)\) \(\envshell_\Delta(\Delta_2)\))}\) fails under \(\delta_1, \delta_2\).
...
\begin{proof}
	...
	\(0\) is multiplicative annihilator, so these cases all result in weight \(0\).
